---
description: "Forge ワークフローシステムの中核ルール。フェーズ規律・エスカレーション・検証ゲートを定義する"
---
...
|-------|--------|----------|
| Quick | Lint | 小さな変更 |
| Standard | Lint + 静的解析 | 通常の変更 |
| Full | Lint + 静的解析 + Tests + Coverage | フェーズ遷移前 |
...
**Priority**: IMPORTANT **Triggers**: フェーズ遷移

ワークフローの各フェーズ間で成果物の品質を確認するための宣言的チェックリスト。上記の Verification Gates（テスト実行レベル）とは異なり、Phase Gates はフェーズ遷移時の成果物の完全性を検証する。
...
- Phase Gates は宣言的なチェックリストであり、自動ブロッキングではない
- 各コマンド定義（brainstorm.md, spec.md, implement.md）からこのセクションを参照する
- spec-validator の Step 0 で brainstorm → spec のエントリー基準を自動検証する
...
1. [ ] 静的解析通過（プロジェクトの静的解析ツールを実行）
2. [ ] テスト通過（プロジェクトのテストを実行）
3. [ ] 変更内容を確認 (`git diff`)
4. [ ] 不要なファイルを除外
5. [ ] 適切なコミットメッセージ
