...
| **レビュー** | `agents/review/` 配下のカスタムレビュアー | 強化 | delta-spec + design.md を必須入力。動的検出 |
...
変更ファイル分析（git diff --stat）
  │
  └── agents/review/ 配下のレビュアーを動的検出し、変更内容とのセマンティックマッチングで選択
```

**コスト削減効果:** 変更に無関係なレビュアーを自動スキップし、トークン使用量を削減。
...
| レビュー見落としパターン | レビュアーのチェックリスト | 「CSRF 対策はセキュリティレビュアーが常にチェック」 |
...
4. 例: 「アーキテクチャレビュアーが直接 DB アクセスを常に指摘するが、このプロジェクトでは設計上許容している」→ 該当レビュアーのチェックリストにプロジェクト固有の例外を追加
...
**採用案: ドメイン検出に基づく動的レビュアー選択（`agents/review/` 配下のレビュアーを自動検出）**
