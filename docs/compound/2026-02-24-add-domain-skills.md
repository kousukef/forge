...
1. **MCP プラグインのセキュリティリスク（P2）**: ui-ux-pro-max-skill の Python スクリプト実行でパストラバーサル・バージョン未固定・入力サニタイズ不足を security-sentinel が検出。MCP サーバー経由の実行推奨・バージョン固定・サニタイズ注記を追加して修正。
