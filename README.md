...
| `/review` | 専門レビュアーによる並列コードレビュー | レビューレポート |
| `/test` | テスト・型チェック・lint・ビルド検証の一括実行 | 実行結果レポート |
| `/compound` | 学びの文書化 + スペックマージ + アーカイブ | `~/.claude/docs/experiential/logs/` |
| `/crystallize` | 蓄積された経験からパターンを抽出し rules/skills/hooks に昇格 | `~/.claude/docs/experiential/patterns/` |
...
| `/handle-pr-review <PR番号> [--no-learn]` | PR レビューコメントの分析・修正・返信を一括処理し、学びを自動記録 |
...
| compound-learnings-researcher | 過去の学びから関連教訓を抽出 | `~/.claude/docs/experiential/logs/` |
...
必須フィールドは `name` と `description` です。`/review` は `description` と変更内容を照合して、関連するレビュアーのみを起動します。
