...
### ステップ6.5: ドキュメント同期ルール設定

スキル検索・インストール完了後、プロジェクト内のドキュメントとコード変更のマッピングルールを対話的に設定する。

#### ドキュメントディレクトリ自動検出

以下のパターンでプロジェクトルートをスキャンし、ドキュメントディレクトリ/ファイルを検出する。検出範囲はルート直下 + 1階層下（ステップ1の技術スタック検出と同じスコープ）。
...
| `CLAUDE.md` | ファイル |
| `AGENTS.md` | ファイル |
| `.claude/` 配下の Markdown ファイル | 特殊 |

#### 検出結果の提示

検出されたドキュメントディレクトリ/ファイルをリスト形式で表示し、ユーザーが追加・削除できる対話を提供する:
...
追加・削除するドキュメントはありますか？ (Enter でそのまま進む)
```

#### マッピング設定の対話

各ドキュメントに対して、どのファイル変更時に更新が必要かを対話的に設定する:

```
docs/ はどのファイル変更時に更新が必要ですか？
例: 「src/ 配下のファイルを変更したら docs/ を確認・更新する」
>
```

ユーザーの回答をもとに、自然言語のマッピングルールを構成する。

#### CLAUDE.md への追記

マッピング設定が完了したら、CLAUDE.md の末尾に `## Document Sync Rules` セクションを追記する。既存セクションの途中には挿入しない。

ルールは自然言語で記述する（YAML 等の構造化形式は使用しない）:
...
CLAUDE.md に既に `## Document Sync Rules` セクションが存在する場合は、既存のルールを表示し、「既存のルールを更新しますか？」とユーザーに確認する。ユーザーが辞退した場合は既存ルールを保持してステップ7に進む。

#### 検出失敗時のフォールバック

検出パターンのいずれにも一致しない場合は「ドキュメントディレクトリが検出されませんでした。手動で指定しますか？」とユーザーに問いかける。ユーザーが辞退した場合はステップ6.5 をスキップしてステップ7に進む。
