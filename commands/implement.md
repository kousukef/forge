...
3. **中間レビューチェックポイント**
   - **トリガー**: tasks.md のタスク総数の50%（切り上げ）が完了した時点で実行。タスク総数が4以下の場合はスキップ
   - **実行回数**: 変更セッションあたり1回のみ
   - **実行内容**: spec-compliance-reviewer を「事後検証モード」で起動し、完了済みタスクのみを対象に検証。プロンプトに「中間レビュー: 完了済みタスクのみを対象に検証してください」と指定する
   - **結果処理**:
     - 逸脱なし: 残タスクの実装を継続
     - 逸脱あり: 逸脱を修正してから残タスクの実装を継続
     - 仕様エスカレーション: SendMessage で Main Agent にエスカレーション → AskUserQuestion でユーザーに確認

4. **Teams 内エスカレーションフロー**
...
2. **中間レビューチェックポイント**
   - **トリガー**: tasks.md のタスク総数の50%（切り上げ）が完了した時点で実行。タスク総数が4以下の場合はスキップ
   - **実行回数**: 変更セッションあたり1回のみ
   - **実行内容**: Task(spec-compliance-reviewer) を「事後検証モード」で起動し、完了済みタスクのみを対象に検証。プロンプトに「中間レビュー: 完了済みタスクのみを対象に検証してください」と指定する
   - **結果処理**:
     - 逸脱なし: 残タスクの実装を継続
     - 逸脱あり: 逸脱を修正してから残タスクの実装を継続
     - 仕様エスカレーション: AskUserQuestion でユーザーに確認

3. **implementer 完了後の検証**
...
Step 5 の検証完了後、ドキュメント同期チェックを実行する。
...
3. セクションが存在する場合: ルールの自然言語記述を取得する
4. Step 5 の `git diff --stat` 出力（既に取得済み）とルールを照合する
5. マッチするルールがある場合: Task(implementer) を doc-sync タスクとして起動する
   - プロンプトに「doc-sync タスク」であることを明記する
   - ルール内容、変更ファイル一覧、対象ドキュメントパスを提供する
   - implementer が更新したドキュメントはワーキングツリーに残る（通常の実装ファイルと同じ）
6. マッチするルールがない場合: `doc-sync: skipped (no matching files)` を出力して Step 6 に進む

**エラーハンドリング:**
- implementer の doc-sync タスクが失敗した場合、エラー内容をログ出力し、ドキュメント更新をスキップして Step 6 に進む。ドキュメント更新失敗は /implement 全体をブロックしない
- 複数のドキュメントを更新中に一部が失敗した場合、成功した更新はワーキングツリーに保持し、失敗したドキュメントのみエラーログに記載して Step 6 に進む
