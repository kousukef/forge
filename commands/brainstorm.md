...
1.5. **ドメインコンテキストの読み込み**（docs/domain/ 参照）:
   - `docs/domain/` ディレクトリの存在を確認する
   - `docs/domain/` が存在しない、または README.md 以外のファイルがない場合はこのステップをスキップする
   - README.md 以外の全ファイルを以下の優先順位で読み込み、ドメインコンテキストを把握する:
     1. `business-rules.md`
     2. `domain-model.md`
     3. `tech-constraints.md`
     4. `stakeholders.md`
     5. `runbooks/` 配下のファイル
   - **読み込み上限: 合計1000行**。1000行に達した時点で残りのファイルはスキップし、「読み込み上限に達したためスキップ: [ファイル名]」と報告する
   - 読み込んだドメイン知識を踏まえて、以降の対話でビジネスルールや技術的制約との整合性を意識する
