...
`openspec/changes/<change-name>/` 配下に以下の4ファイルを生成する:

1. `specs/<feature>/delta-spec.md` -- デルタ要件（ADDED/MODIFIED/REMOVED + Given/When/Then）
2. `design.md` -- リサーチサマリー + 技術設計
3. `tasks.md` -- タスクリスト
4. `traceability.md` -- トレーサビリティマトリクス（US → DD → T → TP → impl → test の双方向追跡）
...
design.md 生成時に「受入テスト計画」セクションを以下の手順で生成する:

1. proposal.md を読み込み、ユーザーストーリー（US-xxx）を抽出する
2. US-xxx 形式で記載されていない場合（自由形式の箇条書き）、箇条書き項目を出現順に US-001, US-002, ... として自動付番する
3. 各 US に対して GIVEN/WHEN/THEN 形式の検証シナリオを生成し、テストレベル指定（L1/L2/L3）を付与する
4. 検証シナリオは「構造的な期待結果」（セクション存在、フォーマット準拠等）を記述する。内容の完全一致は避ける
5. proposal.md が存在しない場合は「受入テスト計画」セクションに「proposal.md が存在しないため受入テスト計画は生成できません」と記載し、design.md の生成は正常に続行する
6. proposal.md にユーザーストーリーが存在しない場合は「ユーザーストーリーが定義されていないため受入テスト計画を生成できません」と記載する

出力形式（後述）に従って4ファイルを生成する。生成順序は delta-spec.md → design.md → tasks.md → traceability.md（traceability.md は他3ファイルの情報を参照するため最後に生成する）。

#### traceability.md の生成

delta-spec.md, design.md, tasks.md の生成が完了した後、以下の手順で traceability.md を生成する:

1. **ID 抽出**: 各成果物から ID を抽出する
   ...
   | TP-xxx | delta-spec.md | 全シナリオ（Happy Path / Error / Boundary）を出現順に連番。複数 feature ディレクトリがある場合はアルファベット順で通し連番 |

2. **Forward Traceability テーブル生成**: US → DD → T → TP のマッピングを行複製方式で記載する（1行につき1つの対応関係。1つの US に複数の T/TP が対応する場合は US を複数行に展開する）
3. **Backward Traceability テーブル生成**: impl/test 列は初期値として空（implementer が更新する）
4. **Coverage Summary 生成**: カテゴリ別の総数/カバー済み/未カバーを集計する
5. **proposal.md 不在時**: proposal.md が存在しない場合は traceability.md の生成をスキップし、警告「proposal.md が見つからないため traceability.md を生成できません」を出力する

#### traceability.md のフォーマット
...
- traceability.md の全 US に対応する T と TP が割り当てられていること
...
- traceability.md: US N件, DD N件, T N件, TP N件
