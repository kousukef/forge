...
- `openspec/changes/<change-name>/traceability.md` -- トレーサビリティマトリクス（網羅性検証用）
...
### 10. トレーサビリティ網羅性チェック（警告レベル）

traceability.md が存在する場合、トレーサビリティマトリクスの網羅性を検証する。この検証は**警告レベル（非ブロッキング）**であり、欠落があっても検証結果は PASS とする。トレーサビリティは補助的な品質指標であり、仕様承認フローをブロックしない。

検証ポイント:
- 全ユーザーストーリー（US-xxx）に対応するタスク（T-xxx）が割り当てられているか
- 全ユーザーストーリー（US-xxx）に対応するテスト観点（TP-xxx）が割り当てられているか
- Coverage Summary の数値（総数/カバー済み/未カバー）が Forward Traceability テーブルの実際のマッピングと一致しているか
- traceability.md が存在しない場合は「警告: traceability.md が生成されていません」と記載し、検証結果は PASS とする

### 11. 受入テスト計画の網羅性チェック

design.md に「受入テスト計画」セクションが存在する場合、その網羅性を検証する。
...
design.md に「受入テスト計画」セクションが存在しない場合は「要修正: 受入テスト計画セクションが design.md に存在しません」とフラグする。特定の US に対応するシナリオが存在しない場合は「要修正: US-xxx に対応する受入テストシナリオが定義されていません」とフラグする。
...
- 受入テスト計画: 全 US カバー済み / 未カバー US あり / セクション不在
- トレーサビリティ網羅性: US N/N カバー済み（警告レベル・非ブロッキング）
```

## ワークフロー

### Step 0: エントリー基準プレチェック

検証開始前に、brainstorm → spec のエントリー基準（`.claude/rules/core-rules.md` Phase Gates 参照）のうち、自動検証可能な項目を確認する。このプレチェックは「11 の検証項目」とは独立した前提条件チェックであり、検証項目数には含めない。

チェック項目:
1. `openspec/changes/<change-name>/proposal.md` が存在するか
2. proposal.md に必須セクションが存在するか:
   - 意図（Intent）セクション
   - ユーザーストーリー / スコープ（Scope）セクション
   - スコープ外（Out of Scope）セクション
3. ユーザーストーリーが最低1つ存在するか
...
各 ADDED / MODIFIED 要件に対して 11 の検証項目を順に実行する。
