...
各指摘には以下の項目を必ず含めること:
- **レビュアー**: 指摘元のレビュアー名（起動されたレビュアーの name を使用）
- **カテゴリ**: 指摘の分類（レビュアーの専門領域に対応するカテゴリ名を動的に使用）
- **指摘内容**: 問題の詳細な説明
...
（カテゴリは起動されたレビュアーに対応するもののみ動的に出力する）

## 矛盾検出
### [CONFLICT-001] [矛盾の概要]
- **レビュアーA**: [レビュアー名] -- [指摘内容と根拠]
- **レビュアーB**: [レビュアー名] -- [指摘内容と根拠]
- **ユーザー判断が必要**: [判断すべきポイント]
...
| 仕様項目 | [レビュアー名1] | [レビュアー名2] | ... | カバー状態 |
|---|---|---|---|---|
| REQ-001: Happy Path | [指摘ID] | - | ... | Covered |
| REQ-001: Error Scenario | - | [指摘ID] | ... | Covered |
| REQ-002: Happy Path | [指摘ID] | - | ... | Covered |
| REQ-002: Boundary | - | - | ... | **UNCOVERED** |

（列は起動されたレビュアー名を動的に列挙する。起動されなかったレビュアーは列に含めない）
...
- カバレッジマトリクスは起動されたレビュアーの列のみ含める（動的検出で未起動のレビュアーは列に含めない）
