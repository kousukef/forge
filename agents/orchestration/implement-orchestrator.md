...
- テストがパスすること: プロジェクトのテストランナーで確認
- 静的解析がパスすること: プロジェクトの静的解析ツールで確認
...
1. **検証コマンド実行**（自身が Bash で実行）:
   - プロジェクトのテストランナー — テスト結果のみ確認
   - プロジェクトの静的解析ツール — 解析エラーの有無のみ確認
...
- 3タスクごとに全テスト実行（プロジェクトのテストランナー）で回帰確認
- 失敗時は `Task(build-error-resolver)` に委譲

### Phase 5: 完了報告

全タスク完了後:
1. プロジェクトのテストランナー + 静的解析ツールで最終検証
2. `git diff --stat` で変更ファイル一覧を取得
3. プロジェクト/グローバル同期確認: 変更ファイルに `~/.claude/` 配下のファイルが含まれる場合、リポジトリ内に対応ファイル（`agents/`, `commands/`, `.claude/rules/`, `reference/` 等）が存在するか確認し、不一致があればコピーによる同期を実施する
