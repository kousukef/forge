...
このシステムはユーザーのプロジェクトに合わせて設定します。特定の技術スタックに依存しません。

- `/setup` コマンドでプロジェクトの技術スタックを自動検出し、適切なドメインスキルをインストール
- 手動で `skills/` にドメインスキルを追加することも可能
- `reference/` にプロジェクト固有のルールを追加可能
...
│   │
│   └── review/                  # レビューフェーズ用（agents/review/ 配下にカスタムレビュアーを配置）
...
```yaml
---
description: "実装済みコードを agents/review/ 配下のレビュアーエージェントで並列レビューする"
---
```

`agents/review/` ディレクトリ内のエージェント定義ファイルを動的にスキャンし、変更内容に関連するレビュアーのみを並列起動する。review-aggregator は常時起動する。
...
4. **E2Eテスト実行**（該当する場合）: プロジェクトの E2E テストランナーを実行
5. **カバレッジ確認**: 80%以上を目標
...
stack: [general | <project-specific-stack>]
...
- 対象フレームワーク: プロジェクトの技術スタック（`/setup` で検出されたもの）に応じて決定
...
新しいレビューエージェントは以下のテンプレートで `agents/review/` に追加する:
...
**チェック項目の例**:
- セキュリティ: OWASP Top 10、シークレット検出、認証・認可の穴
- パフォーマンス: クエリ最適化、バンドルサイズ、キャッシュ戦略
- アーキテクチャ: 責務分離、レイヤー構成、ファイルサイズ制限
- その他プロジェクト固有の観点

`/review` コマンドは `agents/review/` 配下のエージェント定義を動的にスキャンし、変更内容に関連するレビュアーのみを起動する。
...
2. `/setup` コマンドで検索・インストール、または `<project>/.claude/skills/` に手動追加
3. 関連するエージェントの `skills` frontmatter と `## Required Skills` に追加
4. 関連するコマンドの `## Skill Activation` に必要に応じて追記

ドメイン Skill は Auto-Discovery により自動検出される。`/setup` コマンドで技術スタックに応じたスキルを検索・インストール可能。
...
- 認証: ミドルウェアでルートレベルの保護
- 依存関係: パッケージ監査でゼロ脆弱性を維持
```

ドメイン固有のルール（フレームワーク規約、ORM 規約、パフォーマンス規約等）はドメインスキルとして `skills/` に追加するか、`reference/` にプロジェクト固有のルールファイルとして追加してください。
